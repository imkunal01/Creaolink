...
async function computeFeed(userId: string) {
  const db = await getPool();

  // Phase 2: all 5 queries run in parallel — previously sequential.
  const [activity, discover, featured, posts, network] = await Promise.all([
    safeRowsQuery(() =>
      db.query(
  ...
  return { activity, discover, featured, posts, network };
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = buildCacheKey("feed", user.id);

    const data = flags.cacheFeed
      ? await readThroughCache({
          key: cacheKey,
          ttlSeconds: 90,
          source: "api/feed",
          compute: () => computeFeed(user.id),
        })
      : await computeFeed(user.id);
